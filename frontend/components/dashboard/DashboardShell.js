...
export default function DashboardShell({
  ...
  const handleOpenCreateAction = (prob) => {
    setActionTargetProblem(prob || PROBLEMS[0]);
  ...
  return (
    <div className="flex h-screen bg-[#FBF9F5] text-[#18181B] font-sans overflow-hidden antialiased">
      ...
      >
        ...
        <div className="h-16 flex items-center justify-between px-4 border-b border-[#ECE8E0]">
          <div className="flex items-center gap-2.5 overflow-hidden">
            <div className="w-8 h-8 rounded-lg bg-[#18181B] text-white flex items-center justify-center font-serif font-bold text-sm shrink-0">
              R
            </div>
            {!isSidebarCollapsed && (
              <div className="min-w-0">
                <span className="text-sm font-bold tracking-tight font-serif text-[#18181B] block truncate">
                  Reviewr
                </span>
          ...
          <button
            ...
            className="p-1 rounded-md text-[#71717A] hover:bg-[#F4F1EA] hover:text-[#18181B] transition-colors"
        ...
        <div className="p-3 border-t border-[#ECE8E0] bg-[#FAF8F5]">
          <div className="flex items-center gap-2.5">
            <img
              ...
              className="w-8 h-8 rounded-full object-cover border border-[#E5E1D8] shrink-0"
            />
            {!isSidebarCollapsed && (
              <div className="flex-1 min-w-0">
                <p className="text-xs font-bold text-[#18181B] truncate">{USER_PROFILE.name}</p>
                <p className="text-[10px] text-[#71717A] truncate">{USER_PROFILE.title}</p>
              </div>
            )}
            {!isSidebarCollapsed && (
              <button
                onClick={onNavigateLanding}
                className="text-[#71717A] hover:text-[#18181B] p-1 rounded hover:bg-[#EBE7DD]"
        ...
        <header className="h-16 bg-white border-b border-[#E5E1D8] flex items-center justify-between px-4 md:px-8 shrink-0 z-20">
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="md:hidden p-1.5 rounded-lg border border-[#E5E1D8] text-[#71717A]"
            >
            ...
            <button
              ...
              className="flex items-center gap-3 bg-[#FBF9F5] border border-[#E5E1D8] hover:border-[#18181B] rounded-xl px-3.5 py-2 text-xs text-[#71717A] hover:text-[#18181B] transition-all max-w-md w-64 md:w-80 group"
            >
              <Search className="w-3.5 h-3.5 text-[#71717A] group-hover:text-[#18181B]" />
              <span className="flex-1 text-left truncate">Search feedback, problems, evidence...</span>
              ...
              {showNotifications && (
                <div className="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-xl border border-[#E5E1D8] p-3 space-y-2 z-50 animate-in fade-in">
                  <div className="flex items-center justify-between pb-2 border-b border-[#ECE8E0]">
                    <span className="text-xs font-bold text-[#18181B]">Customer Signal Alerts</span>
                    <span className="text-[10px] text-[#71717A]">Real-time</span>
                  </div>
                  <div className="space-y-1.5 max-h-64 overflow-y-auto">
                    {USER_PROFILE.notifications.map((n) => (
                      <div
                        key={n.id}
                        onClick={() => {
          ...
          <div className="max-w-[1360px] mx-auto">
            ...
            {selectedProblemId ? (
              ...
              <>
                ...
                {activeNav === "trends" && (
                  <TrendsView onSelectProblem={handleSelectProblem} />
                )}
                {activeNav === "feedback" && (
                  <FeedbackView onSelectFeedback={handleSelectFeedback} />
                )}
                {activeNav === "sources" && (
                  <SourcesView onOpenConnectSource={() => setIsConnectSourceOpen(true)} />
                )}
